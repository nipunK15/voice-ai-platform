...
// Create + Edit agent with visual flow canvas (React Flow) + detail panel.
...
const CONDITION_LABELS = { keyword: "Keyword", llm_eval: "LLM eval", always: "Always" };
...
function flowToGraph(stages) {
  ...
  stages.forEach((s) => {
    (s.transitions || []).forEach((t, ti) => {
      if (t.toStage) {
        const kwLabel = Array.isArray(t.condition?.keywords)
          ? t.condition.keywords.slice(0, 2).join(", ")
          : typeof t.condition?.keywords === "string"
        ...
        edges.push({
          ...
          source:       s.id,
          target:       t.toStage,
          label:        t.condition?.type === "keyword" ? kwLabel : CONDITION_LABELS[t.condition?.type] || "",
...
function StageNode({ data, selected }) {
  ...
  return (
    <div style={{
      ...
      {stage.goal && (
        <div style={{ fontSize: "12px", color: "#6b7280", marginBottom: "6px" }}>{stage.goal}</div>
      )}
        ...
        <div style={{ marginTop: "8px", display: "flex", flexWrap: "wrap", gap: "4px" }}>
          {stage.transitions.map((t, i) => (
            <span key={i} style={{
              ...
              {CONDITION_LABELS[t.condition?.type] || "?"}
            </span>
          ))}
        </div>
      )}
...
function DetailPanel({ stage, allStages, onChange, onDelete }) {
  if (!stage) {
    return (
      <div style={{
        ...
        textAlign: "center", padding: "32px", gap: "12px",
      }}>
        <div style={{ fontSize: "32px" }}>🎯</div>
        <div style={{ fontSize: "13px", lineHeight: 1.6 }}>
          Click a stage node to edit it,<br />or drag from the green handle<br />to connect stages.
        </div>
      </div>
    );
  }

  const set = (key, val) => onChange({ ...stage, [key]: val });

  const setTransition = (ti, patch) => {
    const t = [...(stage.transitions || [])];
    t[ti] = { ...t[ti], ...patch };
    set("transitions", t);
  };

  const setCondition = (ti, patch) => {
    const t = [...(stage.transitions || [])];
  ...
  const removeTransition = (ti) => {
    set("transitions", (stage.transitions || []).filter((_, i) => i !== ti));
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "14px 18px", display: "flex", flexDirection: "column", gap: "12px" }}>
        ...
        <label style={pLabelStyle}>
          Stage name *
          <input style={pInputStyle} placeholder="e.g. greeting"
            value={stage.name} onChange={(e) => set("name", e.target.value)} />
        </label>

        <label style={pLabelStyle}>
          Stage prompt *
          <textarea style={{ ...pInputStyle, height: "90px", resize: "vertical" }}
        ...
        <label style={pLabelStyle}>
          Goal (optional)
          <input style={pInputStyle} placeholder="e.g. Qualify the lead"
            value={stage.goal || ""} onChange={(e) => set("goal", e.target.value)} />
        </label>

        <label style={pLabelStyle}>
          Auto-advance after N user turns
          <input type="number" min="1" style={{ ...pInputStyle, width: "70px" }}
        ...
        <div>
          ...
          {(stage.transitions || []).map((t, ti) => (
            <div key={ti} style={{
              ...
              <label style={{ ...pLabelStyle, marginBottom: "6px" }}>
                Go to stage
                <select style={{ ...pInputStyle }}
                  value={t.toStage || ""}
                  onChange={(e) => setTransition(ti, { toStage: e.target.value, handoffAssistantId: "" })}>
              ...
              <label style={{ ...pLabelStyle, marginBottom: "6px" }}>
                ...
                <input style={pInputStyle} placeholder="Vapi asst ID"
                  value={t.handoffAssistantId || ""}
                  onChange={(e) => setTransition(ti, { handoffAssistantId: e.target.value, toStage: "" })} />
              </label>

              <label style={{ ...pLabelStyle, marginBottom: "6px" }}>
                Condition
                <select style={pInputStyle}
                  value={t.condition?.type || "keyword"}
                  onChange={(e) => setCondition(ti, { type: e.target.value })}>
              ...
              {t.condition?.type === "keyword" && (
                <label style={pLabelStyle}>
                  ...
                  <input style={pInputStyle} placeholder="yes, confirm, ready"
                    value={Array.isArray(t.condition.keywords)
                      ? t.condition.keywords.join(", ")
                      : t.condition.keywords || ""}
                    onChange={(e) => setCondition(ti, { keywords: e.target.value })} />
                </label>
              )}
              ...
              {t.condition?.type === "llm_eval" && (
                <label style={pLabelStyle}>
                  Evaluation question
                  <textarea style={{ ...pInputStyle, height: "60px", resize: "vertical" }}
                    placeholder="Has the user confirmed their name?"
                    value={t.condition.llmPrompt || ""}
                    onChange={(e) => setCondition(ti, { llmPrompt: e.target.value })} />
                </label>
              )}
              ...
              <button type="button" onClick={() => removeTransition(ti)}
...
export default function CreateAgent() {
  ...
  // Sync stages → canvas
  useEffect(() => {
    const { nodes: n, edges: e } = flowToGraph(stages);
    setNodes(n);
    setEdges(e);
  }, [stages]);

  // Load agent (edit mode)
  useEffect(() => {
    if (!isEdit) return;
    axios.get(`${API}/agents/${id}`)
      .then((res) => {
        ...
        if (a.flowDefinition?.stages?.length) {
          setStages(a.flowDefinition.stages);
          setShowFlow(true);
        }
      })
  ...
  const addStage = () => {
    const s = {
      id: makeStageId(), name: "", prompt: "", goal: "",
  ...
  return (
    <div style={{ padding: "40px 50px", display: "flex", justifyContent: "center" }}>
      <div style={{
        width: "760px", background: "#111827", padding: "40px",
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            <label style={labelStyle}>
              Agent Name *
              <input style={inputStyle} placeholder="e.g. Support Bot"
            ...
            <label style={labelStyle}>
              Personality
              <select style={inputStyle} value={form.personality}
            ...
            <label style={labelStyle}>
              Voice Provider
              <select style={inputStyle} value={form.voiceProvider} onChange={handleProviderChange}>
            ...
            <label style={labelStyle}>
              Voice
              <select style={inputStyle} value={form.voiceId}
              ...
              {showFlow && stages.length > 0 && (
                <>
                  ...
                  <p style={{ fontSize: "12px", color: "#4b5563", margin: "8px 0 0" }}>
                    Click a stage to edit · Drag the green handle → another stage to connect · Drag nodes to rearrange
                  </p>
                </>
              )}
            </div>
...
const labelStyle = {
  ...
  gap: "8px", fontSize: "15px", color: "#d1d5db", fontWeight: 500,
};

const inputStyle = {
  padding: "10px 14px", background: "#1f2937", border: "1px solid #374151",
  borderRadius: "8px", color: "white", fontSize: "15px", outline: "none", width: "100%",
};

const pLabelStyle = {
  display: "flex", flexDirection: "column",
  gap: "5px", fontSize: "12px", color: "#9ca3af", fontWeight: 500,
};

const pInputStyle = {
  padding: "7px 10px", background: "#1f2937", border: "1px solid #374151",
  borderRadius: "6px", color: "white", fontSize: "12px", outline: "none", width: "100%",
};
